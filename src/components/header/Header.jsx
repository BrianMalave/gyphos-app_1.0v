...
const Header = () => {
    ...
    try {
      (async () => {
        ...
        const res = await fetch(
          `${urlTrending}?api_key=${apiKey}&limit=${qty}&rating=g&lang=en`
        );
  ...
  return (
    <header
      className={
        mode ? "header_container night_mode" : "header_container day_mode"
      }
    >
      ...
      <div className={mode ? "head_work night_mode" : "head_work day_mode"}>
        <Link to="/" exact>
          <img
            ...
            src={
              mode
                ? "assets/icons/logo-mobile-modo-noct.svg"
                : "assets/icons/logo-desktop.svg"
            }
            alt="gifos-logo"
          />
        </Link>
        <button className="butn" onClick={setClick}>
          {mode ? way.DAY : way.NIGHT}
        </button>
      </div>
    </header>
  );
